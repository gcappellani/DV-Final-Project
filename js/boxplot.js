...
var margin = { top: 70, right: 40, bottom: 55, left: 180 };
...
d3.csv("https://raw.githubusercontent.com/gcappellani/DV-Final-Project/main/data/eminem.csv",
    ...
    var boxesPerYear = d3.nest()
        .key(d => d.year)
        .rollup(function(d) {
            quartile1 = d3.quantile(d.map(v => v.energy).sort(d3.ascending), .25)
            median = d3.quantile(d.map(v => v.energy).sort(d3.ascending), .5)
            quartile3 = d3.quantile(d.map(v => v.energy).sort(d3.ascending), .75)
            min = d3.min(d.map(v => v.energy))
            max = d3.max(d.map(v => v.energy))
    ...
    xScale.domain(years);
    yScale.domain([0, d3.max(data, function(d) { return d.energy })])
